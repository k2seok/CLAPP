...
public class Server {
	...
	private void dbInit() {
		...
		// 임시 식단
		schedule.setDates("2018-06-11");
		schedao.UpdateSchedule(schedule, "김치찌개/계란말이/햄채소볶음", "사천식잡탕밥/다시마뭇국/모듬깐풍", "");
		schedule.setDates("2018-06-12");
		schedao.UpdateSchedule(schedule, "비빔밥/알찬소시지전/매실차", "육개장칼국수/단호박튀김/겉절이", "");
			...
			public void run() {
				...
				while (true) {
					...
					case "2":// 식단
					{
						...
						System.out.println(schedule.getDates());
						StringBuilder sb = new StringBuilder();
						sb.append("2/A/");
						sb.append(schedule.getMenu1());
						System.out.println("A메뉴 : "+schedule.getMenu1());
						sb.append("/");
						sb.append("B/");
						sb.append(schedule.getMenu2());
						System.out.println("B메뉴 : "+schedule.getMenu2());
						...
						if (select[1].equals("asdf1234s")) {
							...
							if (temp_user == null) {
								...
								if (registerdao.SearchRegister(register) == null) {
									...
									registerdao.UpdateRegister(register, register);
